...
export default {
  ...
  methods: {
    async templateDownload() {
      try {
        const res = await downloadApi.excelMultiImportDownloadTemplate(this.downloadType)
        saveFile(res)
    ...
    // 点击确认
    async confirm() {
      this.$emit('confirm', true, this.fileList)
    },
    cancel() {
      this.$emit('cancel', this.stepsCurrent === 3)
    },
